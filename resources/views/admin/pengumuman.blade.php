...
<style>
    /* Styling yang selaras dengan admin.blade.php */
    .dashboard-header {
        margin-bottom: 30px;
        position: relative;
    }
                            ...
                            <tbody>
                                @forelse($pengumuman as $key => $item)
                                    <tr class="fade-in" style="animation-delay: {{ $key * 0.05 }}s">
                                        ...
                                        <td>{{ $item->title }}</td>
                                        <td>
                                            @if($item->kategori == 'Semua Direktorat')
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                ...
                                                <form action="{{ route('admin.pengumuman.update.status', $item->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PATCH')
                                                ...
                                                <form action="{{ route('admin.pengumuman.destroy', $item->id) }}" method="POST" class="d-inline delete-form">
                                                    @csrf
                                                    @method('DELETE')
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content bounce-in">
            <div class="modal-header">
                <h5 class="modal-title" id="createPengumumanModalLabel"><i class="fa fa-plus-circle me-2"></i>Tambah Pengumuman Baru</h5>
...
@foreach($pengumuman as $item)
    ...
    <div class="modal fade" id="editPengumumanModal{{ $item->id }}" tabindex="-1" aria-labelledby="editPengumumanModalLabel{{ $item->id }}" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content slide-in-up">
                <div class="modal-header">
                    <h5 class="modal-title" id="editPengumumanModalLabel{{ $item->id }}"><i class="fa fa-edit me-2"></i>Edit Pengumuman</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
