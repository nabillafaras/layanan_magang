...
<style>
    /* Styling yang selaras dengan admin1.blade.php */
    .dashboard-header {
        margin-bottom: 30px;
        position: relative;
    }
        ...
        <div class="col-12">
            <div class="dashboard-header fade-in">
                ...
                <ol class="breadcrumb mb-4 slide-in-right">
                    ...
                    <li class="breadcrumb-item active">Manajemen Pengumuman Sekretariat Jenderal</li>
                </ol>
            </div>
            
            <div class="card bounce-in">
                <div class="card-header">
                    <h4><i class="fa fa-bullhorn"></i> Daftar Pengumuman Sekretariat Jenderal</h4>
                            ...
                            <tbody>
                                @forelse($pengumuman as $key => $item)
                                    <tr class="fade-in" style="animation-delay: {{ $key * 0.05 }}s">
                                        ...
                                        <td>{{ $item->title }}</td>
                                        <td>
                                            <span class="badge bg-success">Sekretariat Jenderal</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                ...
                                                <form action="{{ route('admin1.pengumuman1.update.status', $item->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PATCH')
                                                ...
                                                <form action="{{ route('admin1.pengumuman1.destroy', $item->id) }}" method="POST" class="d-inline delete-form">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="text-center">Tidak ada data pengumuman Sekretariat Jenderal</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content bounce-in">
            <div class="modal-header">
                <h5 class="modal-title" id="createPengumumanModalLabel"><i class="fa fa-plus-circle me-2"></i>Tambah Pengumuman Baru - Sekretariat Jenderal</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin1.pengumuman1.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <!-- Menampilkan kategori yang dipilih sebagai informasi saja -->
                    <div class="form-group mb-3">
                        <label>Kategori Pengumuman</label>
                        <input type="text" class="form-control" value="Sekretariat Jenderal" readonly disabled>
                        <small class="form-text text-muted">Pengumuman ini akan ditampilkan khusus untuk Sekretariat Jenderal</small>
                    </div>
...
@foreach($pengumuman as $item)
    ...
    <div class="modal fade" id="editPengumumanModal{{ $item->id }}" tabindex="-1" aria-labelledby="editPengumumanModalLabel{{ $item->id }}" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content slide-in-up">
                <div class="modal-header">
                    <h5 class="modal-title" id="editPengumumanModalLabel{{ $item->id }}"><i class="fa fa-edit me-2"></i>Edit Pengumuman - Sekretariat Jenderal</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('admin1.pengumuman1.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="modal-body">
                        <div class="progress mb-4">
