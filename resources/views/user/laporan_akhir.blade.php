...
<div class="container-fluid p-4">
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-white">
            <h5 class="card-title m-0"><i class="fas fa-file-alt me-2"></i>Laporan Akhir</h5>
        </div>
        <div class="card-body">
            <p class="text-muted">
                Silahkan upload laporan akhir magang Anda di sini. Laporan akhir wajib diupload sebelum masa magang Anda berakhir sebagai syarat penyelesaian program magang.
            </p>
                        ...
                        <div class="accordion-item">
                            ...
                            <div id="collapseUpload" class="accordion-collapse collapse" aria-labelledby="headingUpload" data-bs-parent="#accordionUpload">
                                <div class="accordion-body">
                                    <form action="{{ route('laporan.akhir.upload') }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                    ...
                    <tbody>
                        @forelse($laporan as $index => $item)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('laporan.akhir.download', $item->id) }}" class="btn btn-sm btn-info">
                                        <i class="fas fa-download"></i> Download
                                    </a>
                                    
                                    @if($item->status == 'menunggu')
                                        <form action="{{ route('laporan.akhir.delete', $item->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-3">Belum ada laporan akhir yang diupload</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
